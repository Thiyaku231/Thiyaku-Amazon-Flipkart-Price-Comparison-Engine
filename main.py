...
class Price_compare:
    ...
    def __init__(self, master):
        ...
        label = Label(master, text='Enter The Product Name',font='Helvetica 10 bold')
        label.place(x=60,y=120)
        label.grid(row=4, column=0,padx=(60,40),pady=40)

        entry = Entry(master, textvariable=self.var,font='Helvetica 10 italic')
        entry.grid(row=4, column=1)

        button_find = Button(master, text='Search',font='Helvetica 10 bold', bd=4, command=self.find)
        button_find.grid(row=5, column=1, sticky=W, pady=8)
    ...
    def find(self):
        ...
        self.window = Toplevel(root.config(bg="#5B554F"))
        self.window.title('E-Commerce Price Comparison Engine')
        label_title_flip = Label(self.window, text='Flipkart Title:',font='Helvetica 10 bold')
        label_title_flip.grid(row=0, column=0, sticky=W)

        label_flipkart = Label(self.window, text='Flipkart price (Rs):',font='Helvetica 10 bold')
        ...
        label_title_amzn = Label(self.window, text='Amazon Title:',font='Helvetica 10 bold')
        label_title_amzn.grid(row=3, column=0, sticky=W)

        label_amzn = Label(self.window, text='Amazon price (Rs):',font='Helvetica 10 bold')
        ...
        try:
            ...
            self.variable_amzn.set('Common product not available')
        try:
            self.variable_flip.set(self.matches_flip[0])
        except:
            self.variable_flip.set('Common product not available')
        try:
            option_amzn = OptionMenu(self.window, self.variable_amzn, *self.matches_amzn)
            option_amzn.grid(row=3, column=1, sticky=W)

            lab_amz = Label(self.window, text='Not this product? Try out suggestions by clicking on the title')
            ...
            lab_flip = Label(self.window, text='Not this product? Try out suggestions by clicking on the title')
            ...
            button_amzn_visit = Button(self.window, text='Link to Site', command=self.visit_amzn, bd=4)
            button_amzn_visit.grid(row=4, column=2, sticky=W)

            button_flip_visit = Button(self.window, text='Link to Site', command=self.visit_flip, bd=4)
            button_flip_visit.grid(row=1, column=2, sticky=W)
        except Exception as err:
            print("No Common Products Available")
    def price_flipkart(self, key):
        ...
        try:
            ...
            self.opt_title_flip.set('No Common Product found')
    ...
    def price_amzn(self, key):
        ...
        try:
            ...
            print("No Products Available. Try With other products")
